...
read_R2004_section_map (Bit_Chain *restrict dat, Dwg_Data *restrict dwg)
{
  ...
  int i, error;
  BITCODE_RL section_address, bytes_remaining, checksum, orig_checksum;
  ...
  bytes_remaining = dat->byte; // after decomp
  dat->byte = 0;
...
static Dwg_Section *
find_section (Dwg_Data *dwg, int32_t index)
{
  BITCODE_RL i;
                                ...
                                Dwg_Data *restrict dwg, unsigned int code)
{
  ...
  if (bit_read_H (dat, &ref->handleref))
    {
      LOG_WARN ("Invalid handleref: wanted code %d, got (%d.%d.%lX)", code,
